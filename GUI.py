...
class GUI:
    def __init__(self, world):
        ...
        self.__key_released = True
        self.__mw.bind("<KeyPress>", self.key_event_handler)
        self.__mw.bind("<KeyRelease>", self.key_release)

#        self.__mw.after(10, self.ticker)

    def key_event_handler(self, event):
        if not self.__key_released:
            return
        self.__key_released = False
        self.key_handler(event)

    def key_handler(self, event):
                # TODO: Consider running and cycling
        if not(self.__ongoing_animation or time() - self.__prev_command_timestamp < 0.3 and self.__prev_command == event.keysym):
        ...
        if not self.__key_released:
            return self.__mw.after(10, self.key_handler, event)
